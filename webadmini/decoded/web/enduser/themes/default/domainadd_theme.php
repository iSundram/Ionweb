...
global $user, $globals, $theme, $softpanel, $WE, $error, $domains_list, $domain_types, $ips, $done ;

	softheader(__('Add New Domain'));

	echo '
<div class="col-12 col-lg-10 mx-auto card soft-card p-3">
	<div class="sai_main_head">
		<img src="'.$theme['images'].'adddomain.png" alt="" class="webu_head_img me-2"/>
		<h5 class="d-inline-block">'.__('Add New Domain').'</h5>
	</div>
</div>
...
function suggestdompath(domval) {
    ...
    var domain = $(domval).val().toLowerCase();
    $(domval).val(domain);
...
function handle_wildcard(ele){
	var on = $(ele).prop("checked");
