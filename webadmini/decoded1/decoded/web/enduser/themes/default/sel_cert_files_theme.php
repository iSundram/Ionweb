...
global $user, $globals, $theme, $softpanel, $WE, $error, $done, $wbackup_path, $wbackup_log, $json_files, $backup_path, $type, $backup_data, $full_data, $domainbackups;
	...
	if(optGET('refreshTable') && !empty($done) && optGET('refreshTable') == 'refresh_certBackup'){
		show_table_certs('certs', '.tar.gz');
		return true;
	}
	
	softheader(__('Backuply Certificates'));
...
<div class="card soft-card p-4">';
	backuply_tabs('backupcert');
	echo '
	<div class="tab-content" id="pills-tabContent">
		
		<!-- Certs Now-->
		<div class="tab-pane fade show active" id="backupcert" role="tabpanel" aria-labelledby="backupcert_a">
			<div class="sai_main_head mb-4">
				<img src="'.$theme['images'].'backup_restore.png" alt="" class="mt-2 me-2"/>
				<h5 class="d-inline-block">'. __('Certificates Restore').'</h5>				
				<span type="button" id="refresh_certBackup" onclick="rotate_img(this);" data-tab="certs" class="refresh-icon float-end mt-3" title="'.__('Refresh Table').'">
					<i class="fas fa-sync-alt"></i>
				</span>
			</div>
			<div class="table-responsive certs_table" id="refresh_certBackupDisplay">';
				show_table_certs('certs', '.tar.gz');
			echo '
			</div>
...
function show_table_certs($type, $file_type){
	...
	if(empty($domainbackups[$type])){
		...
		<tr id="'.$db_id.'">
			...
			.($file_data['backup_server_id'] == -1 ? '<td>
				<a href="'.$globals['index'].'act=sel_databaseusers_files&download='.$file_data['filename'].'" id="'.$db_id.'_downloadlink" title="'.__('Download').'" >
	...
	function backup_file(db, dbname) {
		...
		$.ajax({
			...
			success: function(data){
				...
				$("#"+dbname+"_downloadlink").attr("href", "'.$globals['index'].'act=sel_databaseusers_files&download="+data.filedata.filename);
