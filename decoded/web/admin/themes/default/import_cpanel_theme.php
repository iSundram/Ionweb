...
$("#cpanel_scan").click(function(){
var host = $("#host").val();
var cpanel_user = $("#cpanel_user").val();
var cpanel_pass = $("#cpanel_pass").val();
var d = "host="+host+"&cpanel_pass="+encodeURIComponent(cpanel_pass)+"&cpanel_user="+cpanel_user+"&action=listaccts&scan_acc=1";
submitit(d, {
...
localStorage.setItem("allusers", JSON.stringify(response.cpanel_users));
show_users_table();
...
var d = "host="+host+"&cpanel_pass="+encodeURIComponent(cpanel_pass)+"&whm_user="+whm_user+"&import_data="+import_data+"&start_import=1";
submitit(d, {
handle:function(data, p){
var resp = data;
if(resp.error){
...
$("#extra_options").html(\'<div class="row"><div class="col-md-8"><label class="sai_head">'.__js('Create user with :-').'</label>&nbsp;<br><input type="radio" name="plan" id="cpanel_plan" value="1" onclick="show_planlist(`plan`)"/>&nbsp;<label for="cpanel_plan" class="sai_head">'.__js('Create cPanel plan').'</label>&nbsp;&nbsp;<br><input type="radio" name="plan" id="webuzo_plan" value="2" onclick="show_planlist(`plan`)"/>&nbsp;<label for="webuzo_plan" class="sai_head">'.__js('Existing Webuzo plan').'</label>&nbsp;&nbsp;<br><input type="radio" name="plan" id="default_plan" value="3" onclick="show_planlist(`plan`)"/>&nbsp;<label for="default_plan" class="sai_head">'.__js('No plan').'</label></div><div class="col-md-4" id="plan_div" style="display:none"></div></div>\');
}
}
function show_planlist() {
}
planlist = \'<label for="\'+option+\'_name" class="sai_head">'.__js('Select webuzo plan').'</label><br><select name="\'+option+\'_name" id="\'+option+\'_name" >\';
...
if(!empty(owner) && owner != "root" && empty(checked_users[owner]) && !in_array(owner, local_users)){
...
<label for="host" class="sai_head">'.__('cPanel Server Address (Required)').'
...
<div class="col-12 col-md-6 mb-3">
<label for="cpanel_user" class="sai_head">'.__('User Name (Required)').'
<i class="fas fa-info-circle ms-1" data-bs-toggle="tooltip" data-bs-placement="top" title="'.__('WHM User Name').'"></i>
</label>
<input type="text" name="cpanel_user" id="cpanel_user" class="form-control" value="'.POSTval('cpanel_user', '').'" />
</div>
<div class="col-12 col-md-6 mb-3">
<label for="cpanel_pass" class="sai_head ">'.__('Password').'
<i class="fas fa-info-circle ms-1" data-bs-toggle="tooltip" data-bs-placement="top" title="'.__('cPanel user password.<br/>If specified, the process will import data directly from cPanel.').'"></i>
</label>
<div class="input-group password-field mb-3 w-100">
<input type="password" name="cpanel_pass" id="cpanel_pass" class="form-control" value="'.POSTval('cpanel_pass', '').'" />
<span class="input-group-text" onclick="change_image(this, \'cpanel_pass\')">
...
<button id="cpanel_scan" class="flat-butt btn btn-primary"/>'.__('Scan cPanel Server').'</button>
</div>
</div>
</div>
...
<div class="row" id="import_form">
<div class="col-12 col-md-6 mb-3">
<label for="import_hostname" class="sai_head">'.__('cPanel Server Address (Required)').'
<i class="fas fa-info-circle ms-1" data-bs-toggle="tooltip" data-bs-placement="top" title="'.__('IP address or FQDN').'"></i>
</label>
...
<i class="fas fa-info-circle ms-1" data-bs-toggle="tooltip" data-bs-placement="top" title="'.__('Cpanel User Name').'"></i>
</label>
...
<input type="radio" name="plan_backup" id="cpanel_plan1" value="1" onclick="show_planlist(`plan_backup`)"/>&nbsp;
<label for="cpanel_plan1" class="sai_head">'.__('Create cPanel plan').'</label>&nbsp;<br>
