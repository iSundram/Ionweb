...
global $theme, $globals, $user, $langs, $error, $W, $saved, $done, $vendor_info, $rulelist, $modsec_conf;
		
	softheader(__('ModSecurity Rules'));
...
function disable_rule(jEle){
	var id = $(jEle).data("id");
	var status = $(jEle).data("status");
	d = {rule_id : id, rule_status : status, toggle_rule : 1};
	submitit(d,{
		done_reload: window.location
	});
}

function delete_rule(jEle){
	
	a = show_message_r("'.__('Warning').'", "'.__js('Are you sure want to delete ?').'");
	a.alert = "alert-warning";
	
	a.confirm.push(function(){
		var id = $(jEle).data("id");
		var text = $(jEle).data("rule_text");
		d = {rule_id : id, rule_text : text, delete_rule : 1};
		// Submit the data
		submitit(d, {
			done_reload : window.location
		});		
