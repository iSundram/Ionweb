...
global $user, $globals, $theme, $softpanel, $catwise, $error, $done, $dns_list, $domain, $is_running, $ds_algorithms, $ds_digest_types;
	
	$ds_algorithms = ['1-RSAMD5', '2-Diffie-Hellman', '3-DSA/SHA-1', '4-Elliptic Curve', '5-RSA/SHA-1', '7-RSASHA1-NSEC3-SHA1', '8-RSA/SHA-256', '10-RSA/SHA-512', '13-ECDSA Curve P-256 with SHA-256', '14-ECDSA Curve P-384 with SHA-384', '252-Indirect', '253-Private DNS', '254-Private OID'];
	$ds_digest_types = ['1-SHA-1', '2-SHA-256', '4-SHA-384'];
...
function reset_dns(){
	...
	var rdomain = $("#dom_search option:selected").val();
...
function showdns(){
	...
	global $user, $globals, $theme, $softpanel, $catwise, $error, $done, $dns_list, $domain, $domains, $ds_digest_types, $ds_algorithms;
...
function disp_type(str) {
  ...
  // Revert previous form
  revertForm();

  $_("type").innerHTML = str;

  // Hide all form classes
  $(".srv-form, .other-form, .caa-form, .afsdb-form, .hinfo-form, .rp-form, .ds-form").addClass("d-none");

  // Show form based on the selected type
  switch (str) {
    case "SRV":
    ...
    case "CAA":
      ...
      break;
    case "AFSDB":
      $(".afsdb-form").removeClass("d-none");
      break;
    case "HINFO":
      $(".hinfo-form").removeClass("d-none");
      break;
    case "RP":
      $(".rp-form").removeClass("d-none");
      break;
	case "DS":
      $(".ds-form").removeClass("d-none");
      break;
    default:
...
function revertForm(){
	...
	if(previousForm !== ""){
		$(".srv-form, .other-form, .caa-form, .afsdb-form, .hinfo-form, .rp-form, .ds-form").addClass("d-none");
		$("." + previousForm.toLowerCase() + "-form").removeClass("d-none");
	}
}
