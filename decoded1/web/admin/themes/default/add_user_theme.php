...
global $globals, $softpanel, $theme, $_user, $error, $plan_fields, $plans, $done, $SESS, $ips, $storage, $owners, $delegated_ips, $reseller_stats, $resource_total, $resource_limits;
	
	if(optGET('giveform')){
		
		if(optGET('plan') && !empty($plans[optGET('plan')])){
			$plan = $plans[optGET('plan')];
		}else{
			$plan = $_user;
		}
...
<form accept-charset="'.$globals['charset'].'" name="adduserform" id="adduserform" method="post" action=""; class="form-horizontal" onsubmit="return submitit(this)"  data-doneredirect="'.$globals['admin_index'].'act=users">
	...
	<div class="soft-smbox mb-5">
		...
		<div class="sai_form"><br />';
			...
			// Show this option only if cgroups v2 is enabled
...
// Populate ip field on the basis of owner when root logged in
...
function ip_handler(id, ipfield){
	
	$(id).change(function(){
		
		$(ipfield+"_row").show();
		
		var dedicated_selected = 0;
		if($(this).is(":checked")) {
			var dedicated_selected = 1;
		}else{
		...
		$(ipfield+" > option").each(function(){
			...
			// If the IP already belongs to the user somehow, then we will allow
...
$(document).ready(function (){
	...
	// On change of plan, load the form
...
function handleResellerField(){
	var is_reseller = "'.$_user['reseller'].'";
	if(!empty(is_reseller)){
		if (!$("form[name=adduserform] [name=reseller]").is(":checked")) {
			alert("'.__('Warning : The Make Reseller option is unchecked. If you change the plan of a reseller to a plan that does not have the Make Reseller option checked, the reseller will be converted into a normal user and both the reseller and their associated users will be owned by the root user.').'");
		}
	}
...
function handleResellerOwner(){
	var val = $("form[name=adduserform] [name=reseller]").prop("checked");
	var ele = $("[name=owner] [self=1]");
	if(!val){
		ele.hide();
		return;
	}
...
function load_features_form(){
	...
	$.ajax({
		type: "GET",
		url: "'.$globals['admin_url'].'act=add_user&giveform=1&plan="+plan,
		success: function(data){
			...
			// Check if reseller value is set
			handleResellerField();
...
function shift_use_default_div(){
	...
	$("[cb]").each(function(){
		...
		// If we are already inside the values, then return
		if($(this).parent().hasClass("values")){
			return;
		}
...
// Show the other values
function handle_plan_default(ele){
	var par = $(ele).parent().parent();
	var val = ele.checked;
	
	if($("#plan").val().length < 1){
		val = false;
	}
...
function get_form(&$val = NULL, $isplan = NULL){
	
global $globals, $softpanel, $theme, $_user, $error, $plan_fields, $plans, $done;
