...
function manage_root_ssh_keys_theme(){

	global $user, $globals, $theme, $softpanel, $W, $error, $done, $key_list, $_user;
	softheader(__( APP.' - Manage Root SSH Keys'));
	
	echo '
<div class="soft-smbox p-3">
<div class="sai_main_head">
	<img src="'.$theme['images'].'ssh_login.png" alt="" class="webu_head_img me-2"/>
		'.__('Manage Root SSH Keys').'
	</div>
</div>	
<div class="soft-smbox p-4 mt-4">
	<div class="sai_main_head mb-5">
		<div style="float:right;">
		<a type="button" style="text-decoration: none;" class="btn btn-primary" href="'.$globals['admin_index'].'act=ssh_generate_keys" style="margin-right: 10px;">'.__('Generate a New Key').'</a>
		&nbsp;
		<a type="button" style="text-decoration: none;" class="btn btn-primary" href="'.$globals['admin_index'].'act=ssh_import_keys">'.__('Import Key').'</a>		
		</div>
	</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body p-4">
					<form accept-charset="'.$globals['charset'].'" action="" method="post" name="ipblock" id="editform" class="form-horizontal">
						...
						<center>	
							<input type="submit" class="btn btn-primary" value="'.__('Convert').'" name="ppk_gen" id="submitppk" onclick="return submitit(this)" data-donereload="1" />
						</center>
					</form>
				</div>
			</div>
	...
	<div id="showpublickey" class="table-responsive">		
		<table class="table align-middle table-nowrap mb-0 webuzo-table" >			
			<thead class="sai_head2">
				<tr>
					...
					<th class="align-middle" width="12%">'.__('Authorized').'</th>
					<th class="align-middle" width="2%" colspan="3">'.__('Actions').'</th>
				</tr>
		...
		if(empty($key_list['publickeys'])){
			...
			echo '
				<tr id="tr'.$value.'">
					...
					<td width="2%">
						<a href="'.$globals['admin_index'].'act=manage_root_ssh_keys&download='.$value.'" title="'.__('Download').'">
