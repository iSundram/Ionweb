...
class HTTPSocket {
	...
	function query( $request, $content = '', $doSpeedCheck = 0 )
	{
		...
		if ($this->doFollowLocationHeader)
		{
			...
			if (isset($headers['x-use-https']) && $headers['x-use-https']=='yes')
				$this->error[] = $this->ssl_setting_message;
	...
	{
		return json_decode($this->result_body,1);
...
// Create a plan similar to DA
function da_create_plan($plan){
	...
	// r_print($plan);
	if(isset($plan['ssh']) && $plan['ssh'] == 'ON'){
		$data['options']['shell'] =  1;
	}
	
	if(isset($plan['cron']) && $plan['cron'] == 'ON'){
		$data['options']['deny_cron'] =  0;
	}
	
	if(isset($plan['nsubdomains'])){
		$data['max_subdomain'] = $plan['nsubdomains'];
	}
	
	if(isset($plan['bandwidth'])){
		$data['max_bandwidth_limit'] = $plan['bandwidth'];
	}
	
	if(isset($plan['mysql'])){
		$data['max_database'] = $plan['mysql'];
	}
	
	if(isset($plan['ftp'])){
		$data['max_ftp_account'] = $plan['ftp'];
	}
	...
	if(isset($plan['language'])){
		if(!empty($GLOBALS['da_languages'][$plan['language']])){
			$data['lang'] = $GLOBALS['da_languages'][$plan['language']];
		}
	}
	
	if(isset($plan['vdomains'])){
		$data['max_parked_domain'] = $plan['vdomains'];
	}
	...
	if(isset($plan['nemails'])){
		$data['max_email_account'] = $plan['nemails'];
	}
...
function da_user_data_convert($plan){
	
	if(isset($plan['vdomains'])){
		$data['max_parked_domain'] = $plan['vdomains'];
	}
	
	if(isset($plan['suspended_reason'])){
		$data['suspend_reason'] = $plan['suspended_reason'];
	}
	...
	// cpanel is in bytes
	if(isset($plan['bandwidth'])){
		if($plan['bandwidth'] == 'unlimited' || $plan['bandwidth'] == 0){
			$data['max_bandwidth_limit'] = 'unlimited';
		}else{
			$data['max_bandwidth_limit'] = (int) $plan['bandwidth'];
		}
	}
	
	if(isset($plan['package']) && $plan['package'] != 'custom'){
		$data['plan'] = slugify($plan['package']);
	}elseif(isset($plan['original_package']) && $plan['original_package'] != 'custom'){
		$data['plan'] = slugify($plan['original_package']);
	}
	
	if(isset($plan['creator']) && ($plan['creator'] != 'root' && $plan['creator'] != 'admin')){
		$data['owner'] = $plan['creator'];
	}
	...
	if(isset($plan['quota'])){
		$data['max_disk_limit'] = ($plan['quota'] == 'unlimited' ? $plan['quota'] : (int) $plan['quota']);
	}
