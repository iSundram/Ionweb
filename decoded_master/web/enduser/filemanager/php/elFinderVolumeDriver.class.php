...
abstract class elFinderVolumeDriver
{
     ...
     */
    public static function rename_file($src, $target)
    {
        if (!file_exists($src)) {
            return false;
        }

        // If the file is writable, proceed to rename it directly.
        if(is_writable($src)){
            return rename($src, $target);
        }

        // Get original permissions of the source file/directory
        $original_perms = fileperms($src);
        $temp_perms = is_dir($src) ? 0755 : 0644;

        // Change permissions and rename file/directory
        if (chmod($src, $temp_perms) && rename($src, $target)) {
            // Restore original permissions after renaming
