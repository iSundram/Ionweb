...
function sel_cronjob_files_theme(){

global $user, $globals, $theme, $softpanel, $WE, $error, $done, $wbackup_path, $wbackup_log, $json_files, $backup_path, $type, $backup_data, $full_data, $cronbackups;
	
	
	if(optGET('refreshTable') && !empty($done) && optGET('refreshTable') == 'refresh_cronBackup'){
		show_table_cron('crons', '.tar.gz');
		return true;
	}
	
	softheader(__('Backuply Cron Jobs'));
...
<div class="card soft-card p-4">';
	backuply_tabs('backupcronjob');
	echo '
	<div class="tab-content" id="pills-tabContent">
		
		<!-- Cron Now-->
		<div class="tab-pane fade show active" id="backupcronjob" role="tabpanel" aria-labelledby="backupcronjob_a">
			<div class="sai_main_head mb-4">
				<img src="'.$theme['images'].'backup_restore.png" alt="" class="mt-2 me-2"/>
				<h5 class="d-inline-block">'. __('Cron Jobs Restore').'</h5>				
				<span type="button" id="refresh_cronBackup" onclick="rotate_img(this);" data-tab="crons" class="refresh-icon float-end mt-3" title="'.__('Refresh Table').'">
					<i class="fas fa-sync-alt"></i>
				</span>
			</div>
			<div class="table-responsive crons_table" id="refresh_cronBackupDisplay">';
				show_table_crons('crons', '.tar.gz');
			echo '
			</div>
			<div class="text-center" id="msg_backup_cron"></div>
		</div>
		<!-- Cron Now end-->
...
function show_table_crons($type, $file_type){

global $globals, $theme, $l, $softpanel, $backup_path, $cronbackups, $WE;
	echo '
	<script language="javascript" type="text/javascript">
		var filename;
	</script>
<table border="0" cellpadding="6" cellspacing="1" width="100%" class="table align-middle table-nowrap mb-0 webuzo-table">
	...
	if(empty($cronbackups[$type])){
		echo '
		<tr class="text-center">
			<td colspan=11>
				<span>'.__('There are no FTP backups found').' !</span>
			</td>
		</tr>';
	}else{
		foreach($cronbackups[$type] as $db => $dbinfo){
