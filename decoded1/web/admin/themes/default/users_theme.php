...
global $user, $globals, $theme, $softpanel, $error, $done, $users, $SESS, $resellers, $title;
...
function del_user(el){
	...
	var reseller_exist = "";
	var resellers = ["'.implode('", "', $resellers).'"];
		
	$("input:checkbox[name=checked_user]:checked").each(function(){
		var user = $(this).val();
		arr.push($(this).val());
		
		// Check if selected users has resellers
		if(in_array(user, resellers)){
			reseller_exist = 1;
		}
	});
...
function reset_reseller_pri(el){
	var a;
	a = show_message_r("'.__js('Warning').'", "'.__js('Are you sure you want to Reset Privileges for the selected reseller(s) ?').'");
	a.alert = "alert-warning";
	
	var resuser = [];
	$("input:checkbox[name=checked_user]:checked").each(function(){
		resuser.push($(this).val());
	});
...
function reset_bandwidth_usage(ele){
	var jEle = $(ele);
	var d = jEle.data();
	var a, lan;
	var username = d.user;
	
	lan = "'.__js('Are you sure you want to $0 reset Bandwidth Usage $1 of ', ['<b>', '</b>']).'<b>"+username+"</b>";
	...
	a.confirm.push(function(){
		AJAX({				
			type: "POST",
			url: "'.$globals['admin_url'].'act=view_bandwidth&api=json",
			data: {
				r_user: username,
				reset_usage: 1,
			},
...
function suspend_user(el){
	var jEle = $(el);
	var a;
	var suspend_check = "<br /><input type=\"radio\" name=\"suspend_main\" value=\"2\" />&nbsp;&nbsp;<b>'.__js('Suspend only reseller main account').'</b>"+
	"<br /><input type=\"radio\" name=\"suspend_main\" value=\"1\" />&nbsp;&nbsp;<b>'.__js('Suspend only reseller sub-accounts').'</b>"+
	"<br /><input  class=\"mb-3\" type=\"radio\" name=\"suspend_main\" value=\"0\" checked />&nbsp;&nbsp;<b>'.__js('Suspend both reseller and sub-accounts').'</b>";
	
	var prevent_unsuspend = "<input class=\"mb-3\" type=\"checkbox\" name=\"prevent_unsuspend\" id=\"prevent_unsuspend\" />&nbsp;&nbsp;"+"'.__js('$0 Prevent unsuspend ? $1', ['<b>', '</b>']).'<i class=\"fas fa-info-circle\" data-bs-toggle=\"tooltip\" data-bs-placement=\"top\" title=\"'.__("Reseller with all privileges can unsuspend the user but if checked then only root is allowed to unsuspend the user.").'\"></i>";
	
	// console.log(d);return false;
	if(jEle.data().resl == 1){
		var lang = prevent_unsuspend + suspend_check + "<br />'.__js('Are you sure you want to suspend this user ?').' <br />";
	}else{
...
function unsuspend_user(el){
	...
	var a;
	var jEle = $(el);
	var unsuspend_check = "<br /><input type=\"radio\" name=\"unsuspend_main\" value=\"2\" />&nbsp;&nbsp;<b>'.__js('Unsuspend only reseller main account').'</b>"+
	"<br /><input type=\"radio\" name=\"unsuspend_main\" value=\"1\" />&nbsp;&nbsp;<b>'.__js('Unsuspend only reseller sub-accounts').'</b>"+
	"<br /><input type=\"radio\" name=\"unsuspend_main\" value=\"0\" checked />&nbsp;&nbsp;<b>'.__js('Unsuspend both reseller and sub-accounts').'</b>";
	// console.log(d);return false;
	if(jEle.data().resl == 1){
...
function suspend_email(el){
	var jEle = $(el);
	var a;

	var email_check = `<br />
		<input type="radio" name="suspend_email_check" id="suspend_email_check" checked value="all" />&nbsp;&nbsp;<b>'.__js('Suspend all emails').'</b>
		<br /><input type="radio" name="suspend_email_check" id="suspend_email_check" value="script" />&nbsp;&nbsp;<b>'.__js('Suspend only script emails').'</b>`;

	var suspend_check = "<br /><br /><input type=\"radio\" name=\"suspend_main\" value=\"2\" />&nbsp;&nbsp;<b>'.__js('Suspend only reseller main account emails').'</b>"+
	"<br /><input type=\"radio\" name=\"suspend_main\" value=\"1\" />&nbsp;&nbsp;<b>'.__js('Suspend only reseller sub-accounts emails').'</b>"+
	"<br /><input type=\"radio\" name=\"suspend_main\" value=\"0\" checked />&nbsp;&nbsp;<b>'.__js('Suspend both reseller and sub-accounts emails').'</b>";

	if(jEle.data().resl == 1){
		var lang = "'.__js('Are you sure you want to suspend Emails as per selected options ?').'" + email_check + suspend_check;
	}else{
...
function unsuspend_email(el){
	
	var a;
	var jEle = $(el);
	var unsuspend_check = "<br /><input type=\"radio\" name=\"unsuspend_main\" value=\"2\" />&nbsp;&nbsp;<b>'.__js('Unsuspend only reseller main account emails').'</b>"+
	"<br /><input type=\"radio\" name=\"unsuspend_main\" value=\"1\" />&nbsp;&nbsp;<b>'.__js('Unsuspend only reseller sub-accounts emails').'</b>"+
	"<br /><input type=\"radio\" name=\"unsuspend_main\" value=\"0\" checked />&nbsp;&nbsp;<b>'.__js('Unsuspend both reseller and sub-accounts emails').'</b>";
	// console.log(d);return false;
	if(jEle.data().resl == 1){
		var lang = "'.__js('Are you sure you want to unsuspend Email as per selected options ?').'" + unsuspend_check;
	}else{
...
function delete_reseller(el){
	...
	var del_res_main_check = "<br /><input type=\"checkbox\" name=\"del_r_main\" id=\"del_r_main\" checked />&nbsp;&nbsp;"+"'.__js('$0 Delete reseller main account $1', ['<b>', '</b>']).'";
	var del_res_user_check = "<br /><input type=\"checkbox\" name=\"del_r_users\" id=\"del_r_users\" checked />&nbsp;&nbsp;"+"'.__js('$0 Delete all users account of reseller $1', ['<b>', '</b>']).'";
...
function reselleradmin(username) {
	$.ajax({
		type: "POST",
		url: "'.$globals['admin_url'].'act=sso&api=json",
		data: {
			user: username
		},
