...
function cronjob_theme(){
	...
	global $theme, $globals, $error, $softpanel ,$done, $user, $WE;
...
<div class="card soft-card p-3">
	<div class="sai_main_head">
		<img src="'.$theme['images'].'cronjob.png" alt="" class="webu_head_img me-2"/>
		<h5 class="d-inline-block">'.__('Cron Job').'</h5>
		<button type="button" class="flat-butt float-end" data-bs-toggle="modal" data-bs-target="#cronModal">'.__('Add Cron Job').'</button>
	</div>
</div>
		...
		<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form accept-charset="'.$globals['charset'].'" action="" method="post" name="cronjob" id="create_cron" onsubmit="return submitit(this)" data-donereload="1">
					<div class="mb-3">
						...
						<label class="form-label d-block" for="minute">'.__('Minute').'						
							<i class="fas fa-info-circle ms-1" data-bs-toggle="tooltip" data-bs-placement="top" title="'.__('[0-59]').'"></i>
						</label>
						<input type="text" name="minute" id="minute" class="display-value d-inline-block" value="  * " />
						<select id="minute_options"class="selected-value d-inline-block mb-2" onchange="select_option(\'minute\')">
							<option value="---">---'.__('Common Settings').'---</option>
...
global $user, $globals, $theme, $softpanel, $WE, $catwise, $error, $done, $domain_list, $read_cron, $domain_name;
	

	echo '	
		
<table border="0" cellpadding="8" cellspacing="1" class="table align-middle table-nowrap mb-0 webuzo-table">			
<thead class="sai_head2">
	<tr>
		...
		<th class="text-center" colspan="3">'.__('Option').'</th>
	</tr>
</thead>
<tbody>';
...
function del_cron(el){
	var a;
	var jEle = $(el);
	var arr = [];
	
	$("input:checkbox[name=check_cron]:checked").each(function(){
		var cron = $(this).val();
		arr.push(cron);
	});
