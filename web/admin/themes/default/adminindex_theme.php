...
global $globals, $softpanel, $theme, $user, $count_data, $sitepad_installed, $SESS, $usage, $info, $fav_pref, $fav_list, $menu_list, $acts, $admin_menu, $reseller_menu;
	
	if(optGET('rebuild_cache')){
			...
			pie: { 
				...
				label: {
					...
					formatter: function(label, series){
						...
						return \'<div style="font-size:18px; font-weight:600;">\'+Math.round(series.percent)+\'%</div><div style="font-size:10px; color:#333;">\'+label+\'</div>\';	
					}
				},
...
function load_soft_info(){
	...
	if(typeof(soft_news) == "undefined"){
		$_("softnews").innerHTML = "'.__js('Unable to connect to '.APP).'";
	}else{
		var newsstr = "";
		for(x in soft_news){
