...
global $user, $globals, $theme, $softpanel, $WE, $error, $done, $domains, $install_lelist, $domain_name, $le_log, $email_conf, $webuzo_conf, $selected_domain;
...
function handle_action_buts() {
	if($(".cer_check:checked").length){
		$("#sel_ins").removeAttr("disabled");
		$("#sel_ren").removeAttr("disabled");
		$("#sel_rev").removeAttr("disabled");
	}else{
...
function cert_action(ele){
	submitit($(ele).data());
}

function with_selected_cert(action){
	var domains = [];
...
function open_cer(dom){
	...
	$.ajax({				
		...
		success: function(domain){
			$(".loading").hide();
			$("#showrectab").html(domain);
			var bm = bootstrap.Modal.getOrCreateInstance($("#view_certificate")[0]);	
			bm.show();
		}															  
	});
}
...
// get Lets Ecrypt logs (and clear them if needed)
function get_logs(jEle){
	...
	return submitit(d, {
		handle: function(data){
			...
			if("done" in data){
				if("clearlog" in d){
					m = show_message_r("Delete", "Logs Cleared");
...
global $user, $globals, $theme, $softpanel, $WE, $error , $done, $domains, $domain_name, $install_lelist;

echo '
<div class="modal fade" id="view_certificate" tabindex="-1" aria-labelledby="view_certificateLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title action-type" id="view_certificateLabel">'.__('Certificate Details').'</h5>
			...
			<div class="modal-body p-4">
				<table class="table align-middle table-nowrap mb-0 webuzo-table td_font">
					...
					if(!empty($install_lelist[$domain_name])){
						...
						<tr>
							<td>'.__('Domain').'</td>
							<td>'.$install_lelist[$domain_name]['cert_info']['subject'].'</td>
						</tr>
						<tr>
							<td>'.__('Issued By').'</td>
							<td>'.$install_lelist[$domain_name]['cert_info']['issuer'].'</td>
						</tr>
						<tr>
							<td>'.__('Version').'</td>
							<td>'.$install_lelist[$domain_name]['cert_info']['version'].'</td>
						</tr>
						<tr>
							<td>'.__('Serial No.').'</td>
							<td>'.$install_lelist[$domain_name]['cert_info']['serial'].'</td>
						</tr>
						<tr>
							<td>'.__('Valid From').'</td>
							<td>'.$install_lelist[$domain_name]['cert_info']['val_from'].'</td>
						</tr>
						<tr>
							<td>'.__('Valid Till').'</td>
							<td>'.$install_lelist[$domain_name]['cert_info']['val_till'].'</td>
						</tr>
						<tr>
							<td>'.__('Type').'</td>
							<td>'.$install_lelist[$domain_name]['cert_info']['type'].'</td>
						</tr>
						<tr>
							<td>'.__('Created on').'</td>
							<td>'.$install_lelist[$domain_name]['cert_info']['created'].'</td>
						</tr>
						<tr>
							<td>'.__('Next Renew Date').'</td>
							<td>'.$install_lelist[$domain_name]['cert_info']['next_renew'].'</td>
						</tr>
						<tr>
							<td>'.__('Certificate Domains').'</td>
							<td>'.implode('<br />', $install_lelist[$domain_name]['cert_info']['cert_domains']).'</td>
						</tr>';
					}else{
