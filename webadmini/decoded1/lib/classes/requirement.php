...
class requirement{
	...
	function req_version($param){
		...
		global $globals, $error, $softpanel, $__settings;
	...
	function remote_mysql_version_check($params){
		...
		foreach($params as $k => $param){
			...
			if(empty($param['err'])){		
				$param['err_key'] = 'req_ver_nf_'.$param['check'];
					$param['err_val'] = __('Required $0 version $1 $2 AND found version is : ', array(strtoupper($param['check']), $optr[$param['operator']], $param['version']));
			}else{
				$param['err_key'] = $param['err'];
				$param['err_val'] = '{{'.$param['err'].'}}';
			}
