...
global $globals, $theme, $dns_list, $is_running, $dns_zones, $error;
...
function set_ttl_sel(el){
	var domains = [];
	var up_ttl = $("#ttl_sel").val();
	var check = 0;
	
	$(".ttlcheck").each(function(){
		if($(this).is(":checked")){
			check = 1;
			return false;
		}
...
function showdns(){
	...
	global $dns_list, $dns_zones;
	
	echo '
<table class="table webuzo-table" >			
<thead>
	<tr>
		<th width="1%"><input type="checkbox" id="check_all_ttl"></th>
		<th>'.__('Name').'</th>
		<th>'.__('User').'</th>
		<th>'.__('Owner').'</th>
		<th>'.__('TTL').'</th>
		<th width="8%">'.__('Option').'</th>
	</tr>
</thead>
<tbody>';
