...
function sel_ftp_files_theme(){

global $user, $globals, $theme, $softpanel, $WE, $error, $done, $wbackup_path, $wbackup_log, $json_files, $backup_path, $type, $backup_data, $full_data, $ftpbackups;
	
	
	if(optGET('refreshTable') && !empty($done) && optGET('refreshTable') == 'refresh_ftpBackup'){
		show_table_ftps('ftps', '.tar.gz');
		return true;
	}
	
	softheader(__('Backuply FTPs'));
...
<div class="card soft-card p-4">';
	backuply_tabs('backupftp');
	echo '
	<div class="tab-content" id="pills-tabContent">
		
		<!-- FTPs Now-->
		<div class="tab-pane fade show active" id="backupftp" role="tabpanel" aria-labelledby="backupftp_a">
			<div class="sai_main_head mb-4">
				<img src="'.$theme['images'].'backup_restore.png" alt="" class="mt-2 me-2"/>
				<h5 class="d-inline-block">'. __('FTP Restore').'</h5>				
				<span type="button" id="refresh_ftpBackup" onclick="rotate_img(this);" data-tab="ftps" class="refresh-icon float-end mt-3" title="'.__('Refresh Table').'">
					<i class="fas fa-sync-alt"></i>
				</span>
			</div>
			<div class="table-responsive ftps_table" id="refresh_ftpBackupDisplay">';
				show_table_ftps('ftps', '.tar.gz');
			echo '
			</div>
			<div class="text-center" id="msg_backup_ftp"></div>
		</div>
		<!-- FTPs Now end-->
...
function show_table_ftps($type, $file_type){

global $globals, $theme, $l, $softpanel, $backup_path, $ftpbackups, $WE;
	echo '
	<script language="javascript" type="text/javascript">
		var filename;
	</script>
<table border="0" cellpadding="6" cellspacing="1" width="100%" class="table align-middle table-nowrap mb-0 webuzo-table">
	...
	if(empty($ftpbackups[$type])){
		...
		foreach($ftpbackups[$type] as $db => $dbinfo){
