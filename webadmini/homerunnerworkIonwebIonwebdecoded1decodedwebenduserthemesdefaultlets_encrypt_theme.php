			...
			<div class="modal-body p-4">
				<table class="table align-middle table-nowrap mb-0 webuzo-table td_font">
					...
					$ll = array(
						...
						'version' => __('Version'),
						'serial' => __('Serial No.'),
						'val_from' => __('Valid From'),
						'val_till' => __('Valid Till'),
						'type' => __('Type'),
						'cert_domains' => __('Certificate Domains'),
						'created' => __('Created on'),
						'not_created' => __('NA'),
						'next_renew' => __('Next Renew Date')
					);
					if(!empty($install_lelist[$domain_name])){
						foreach($install_lelist[$domain_name]['cert_info'] as $k => $v){
							...
							<tr>
								<td>'.$ll[$k].'</td>
								<td>'.(is_array($v) ? implode('<br />', $v) : $v).'</td>
							</tr>';
						}
					}else{
