...
function sel_email_files_theme(){

global $user, $globals, $theme, $softpanel, $WE, $error, $done, $wbackup_path, $wbackup_log, $json_files, $backup_path, $type, $backup_data, $full_data, $emailbackups;
	
	
	if(optGET('refreshTable') && !empty($done) && optGET('refreshTable') == 'refresh_mailBackup'){
		show_table_emails('emails', '.tar.gz');
		return true;
	}
	
	softheader(__('Backuply Email'));
...
<div class="card soft-card p-4">';
	backuply_tabs('backupmail');
	echo '
	<div class="tab-content" id="pills-tabContent">
		
		<!-- Email Now-->
		<div class="tab-pane fade show active" id="backupmail" role="tabpanel" aria-labelledby="backupmail_a">
			<div class="sai_main_head mb-4">
				<img src="'.$theme['images'].'backup_restore.png" alt="" class="mt-2 me-2"/>
				<h5 class="d-inline-block">'. __('Email Restore').'</h5>				
				<span type="button" id="refresh_mailBackup" onclick="rotate_img(this);" data-tab="emails" class="refresh-icon float-end mt-3" title="'.__('Refresh Table').'">
					<i class="fas fa-sync-alt"></i>
				</span>
			</div>
			<div class="table-responsive emails_table" id="refresh_mailBackupDisplay">';
				show_table_emails('emails', '.tar.gz');
			echo '
			</div>
			<div class="text-center" id="msg_backup_mail"></div>
		</div>
		<!-- Email Now end-->
...
function show_table_emails($type, $file_type){

global $globals, $theme, $l, $softpanel, $backup_path, $emailbackups, $WE;
	echo '
	<script language="javascript" type="text/javascript">
		var filename;
	</script>
<table border="0" cellpadding="6" cellspacing="1" width="100%" class="table align-middle table-nowrap mb-0 webuzo-table">
	...
	if(empty($emailbackups[$type])){
		echo '
		<tr class="text-center">
			<td colspan=11>
				<span>'.__('There are no email backups found').' !</span>
			</td>
		</tr>';
	}else{
		foreach($emailbackups[$type] as $db => $dbinfo){
