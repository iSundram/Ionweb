...
global $user, $globals, $theme, $softpanel, $apps, $iapps, $error;
global $software, $soft, $info, $settings, $init_tab, $done, $__settings, $notes, $cron, $overwrite_option, $nopackage, $install, $taskID;
...
<?php
	...
	if(empty($globals['off_rating_link'])){
		echo '$_(\'ratings_js_file\').src = "https://api.webuzo.com/ratingsjs.php?soft='.$soft.'&ip='.$_SERVER['SERVER_ADDR'].'";';
	}
...
function notified(re){
	if(re == 1){
		alert('<?php echo __('Thank you for informing about the new version. We will look into it as soon as possible') ?>');
	}
};
	...
	<div class="col-12 col-md-10">
		...
		<p align="center">';
			...
			if(empty($iapps[$soft.'_1'])) {
				...
				// TODO : Checkbox to show remove dependency and skip-dep
		...
		function confirm_remove(){
			...
			var r = confirm("'.__js('Are you sure you wish to remove this application ? The action will be irreversible. \\nNo further confirmations will be asked.').'");
			if(r != true){
				return false;
			}
		...
		function confirm_reinstall(){
			...
			var r = confirm("'.__js('Are you sure you wish to Re-install this application ? The action will be irreversible. \\nNo further confirmations will be asked.').'");
		...
		function get_package(){	
			if(nopackage){
				...
				show_msg("'.__('Downloading Package').'");
		...
		function get_package_handle(resp){
			nopackage = 0;
			show_msg("'.__('Installing Application').'");
			checkform(1);
		};
